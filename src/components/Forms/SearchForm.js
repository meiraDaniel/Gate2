...
function SearchForm({ places, filter_places, handleFilter, isLoading }) {
  ...
  useEffect(() => {
    const filter_tourNames = places.filter((data) =>
      data.tour_name.toLowerCase().includes(inputFilterValue.toLowerCase())
    );
    const filter_activity = places.filter((data) =>
      data.activities.toString().includes(inputFilterValue.toLowerCase())
    );
    const filter_destination = places.filter((data) =>
      data.place
        .toLowerCase()
        .split(",")[0]
    ...
    const filter_activities_tours = filter_tourNames.concat(
      filter_activity.filter((item) => filter_tourNames.indexOf(item) < 0)
    );
    const filter_inputSearch = filter_activities_tours.concat(
      filter_destination.filter(
        (item) => filter_activities_tours.indexOf(item) < 0
      )
    );

    const filter_places = filter_inputSearch.filter(
      (data) =>
        data.price >= lowPrice &&
        data.price <= highPrice &&
        data.number_days >= lowDay &&
        data.number_days <= highDay
    );
